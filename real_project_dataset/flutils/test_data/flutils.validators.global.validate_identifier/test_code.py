def test_integration_validate_identifier(self):
        try:
            ...
            self.fail("'validate_identifier()' raised TypeError unexpectedly! %s" % e)
        except SyntaxError as e:
            self.fail("'validate_identifier()' raised SyntaxError unexpectedly! %s" % e)
...
def test_integration_validate_identifier_invalid_type(self):
        vals = None, 33, 33.33, list(), tuple(), SimpleNamespace(), dict(a=5)
        for val in vals:
...
def test_integration_validate_identifier_builtin_raises(self):
        names = tuple(
            filter(
                lambda x: x.startswith("__") and x.endswith("__"), dir("__builtins__")
            )
        )
        for val in names:
            with self.assertRaises(SyntaxError):
