        ...
        for args, exp in self.values:
            ...
            with self.subTest(**kwargs):
                kwargs["ret"] = bump_version(
                    args[0], position=args[1], pre_release=args[2]
                )
                self.assertEqual(
                    kwargs["ret"],
                    exp,
...
def test_bump_version__00(self) -> None:
        ...
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
        build_version_info = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.packages._build_version_bump_position", return_value=position
        )
        build_version_bump_position = patcher.start()
        self.addCleanup(patcher.stop)
        patcher = patch(
            "flutils.packages._build_version_bump_type", return_value=bump_type
        )
...
def test_bump_version__02(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            minor=_VersionPart(
                pos=1, txt="2a19", num=2, pre_txt="a", pre_num=19, name="minor"
            ),
            patch=_VersionPart(
                pos=2, txt="", num=0, pre_txt="", pre_num=-1, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
...
def test_bump_version__04(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            minor=_VersionPart(
                pos=1, txt="2a0", num=2, pre_txt="a", pre_num=0, name="minor"
            ),
            patch=_VersionPart(
                pos=2, txt="", num=0, pre_txt="", pre_num=-1, name="patch"
            ),
...
def test_bump_version__10(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            minor=_VersionPart(
                pos=1, txt="140", num=140, pre_txt="", pre_num=-1, name="minor"
            ),
            patch=_VersionPart(
                pos=2, txt="2b20", num=2, pre_txt="b", pre_num=20, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
...
def test_bump_version__13(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            patch=_VersionPart(
                pos=2, txt="1b10", num=1, pre_txt="b", pre_num=10, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
...
def test_bump_version__14(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            patch=_VersionPart(
                pos=2, txt="1a137", num=1, pre_txt="a", pre_num=137, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
...
def test_bump_version__16(self) -> None:
        ...
        ver_info = _VersionInfo(
            ...
            patch=_VersionPart(
                pos=2, txt="1b137", num=1, pre_txt="b", pre_num=137, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
...
def test_bump_version__17(self) -> None:
        ...
        ver_info = _VersionInfo(
            version=version,
            major=_VersionPart(
                pos=0, txt="7", num=7, pre_txt="", pre_num=-1, name="major"
            ),
            minor=_VersionPart(
                pos=1, txt="6", num=6, pre_txt="", pre_num=-1, name="minor"
            ),
            patch=_VersionPart(
                pos=2, txt="14", num=14, pre_txt="", pre_num=-1, name="patch"
            ),
            pre_pos=-1,
        )
        patcher = patch("flutils.packages._build_version_info", return_value=ver_info)
