...
def test_list(self) -> None:
        obj_dict = {"b": 2, "a": 1}
        obj_list = [obj_dict, 3, obj_dict]
        make = namedtuple("NamedTuple", ("a", "b"))
        obj_ntup = make(1, 2)
        val = [obj_ntup, 3, obj_ntup]
...
def test_tuple(self) -> None:
        obj_dict = {"b": 2, "a": 1}
        obj_list = obj_dict, "test", obj_dict
        make = namedtuple("NamedTuple", ("a", "b"))
        obj_ntup = make(1, 2)
        val = obj_ntup, "test", obj_ntup
        res = to_namedtuple(obj_list)
        self.assertEqual(res, val)
...
def test_complex(self) -> None:
        arg = {
            ...
            "a": 1,
            "c": {"a": 1, "b": 2},
            "d": [({"a": 1, "b": 2}, {"a": 1, "b": 2}), {"a": 1, "b": 2}, "test"],
        }
        make = namedtuple("NamedTuple", ("a", "b"))
        obj_ntup = make(1, 2)
        exp_list = [(obj_ntup, obj_ntup), obj_ntup, "test"]
        make = namedtuple("NamedTuple", ("a", "b", "c", "d"))
        exp = make(1, 2, obj_ntup, exp_list)
        res = to_namedtuple(arg)
        self.assertEqual(res, exp)